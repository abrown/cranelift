...
pub fn define(shared: &mut SharedDefinitions, x86_instructions: &InstructionGroup) {
    ...
    for ty in ValueType::all_lane_types().filter(|t| t.lane_bits() == 8) {
        let splat_any8x16 = splat.bind_vector_from_lane(ty);
        let bitcast_f64_to_any8x16 = bitcast.bind_vector_from_lane(ty).bind(F64);
        narrow.legalize(
    ...
    for ty in ValueType::all_lane_types().filter(|t| t.lane_bits() == 16) {
        let splat_x16x8 = splat.bind_vector_from_lane(ty);
        let raw_bitcast_any16x8_to_i32x4 = raw_bitcast
            .bind_vector_from_lane(I32)
            .bind_vector_from_lane(ty);
    ...
    for ty in ValueType::all_lane_types().filter(|t| t.lane_bits() == 32) {
        let splat_any32x4 = splat.bind_vector_from_lane(ty);
        narrow.legalize(
    ...
    for ty in ValueType::all_lane_types().filter(|t| t.lane_bits() == 64) {
        let splat_any64x2 = splat.bind_vector_from_lane(ty);
        narrow.legalize(
