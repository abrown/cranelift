...
// parsing will need to generate a boxed version of a Uimm128 in order to fit inside the 16-byte InstructionData
impl FromStr for Box<Uimm128> {
    type Err = &'static str;

    // Parse a decimal or hexadecimal `Uimm32`, formatted as above.
    fn from_str(s: &str) -> Result<Self, &'static str> {
        let uimm128: Uimm128 = s.parse()?;
        Ok(Box::new(uimm128))
    }
}
