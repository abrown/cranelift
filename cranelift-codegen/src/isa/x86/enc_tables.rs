...
fn expand_minmax(
    ...
    if func.dfg.value_type(x).is_vector() {
        let mut pos = FuncCursor::new(func).at_inst(inst);
        pos.use_srcloc(inst);

        let comparison = pos.ins().fcmp(FloatCC::UnorderedOrGreaterThan, x, y); // TODO cc code
                                                                                // incorrect
                                                                                // TODO just use https://github.com/v8/v8/blob/master/src/compiler/backend/x64/code-generator-x64.cc#L2549-L2568
    } else {
        let old_ebb = func.layout.pp_ebb(inst);
