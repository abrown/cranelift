...
use cranelift_codegen::ir::{self, ConstantData, InstBuilder, JumpTableData, MemFlags, ValueLabel};
...
pub fn translate_operator<FE: FuncEnvironment + ?Sized>(
    ...
    match op {
        ...
        Operator::I16x8Shl | Operator::I32x4Shl | Operator::I64x2Shl => {
            ...
            let result = builder.ins().ishl(a, b_mod_bitwidth);
            state.push1(result)
        }
        Operator::I16x8ShrU | Operator::I32x4ShrU | Operator::I64x2ShrU => {
            ...
            // we do `b AND 15`; this means fewer instructions than `iconst + urem`.
            let b_mod_bitwidth = builder.ins().band_imm(b, bitwidth - 1);
            let result = builder.ins().ushr(a, b_mod_bitwidth);
            state.push1(result)
        }
        Operator::I16x8ShrS | Operator::I32x4ShrS => {
            ...
            // we do `b AND 15`; this means fewer instructions than `iconst + urem`.
            let b_mod_bitwidth = builder.ins().band_imm(b, bitwidth - 1);
            let result = builder.ins().sshr(a, b_mod_bitwidth);
            state.push1(result)
        }
        Operator::V128Bitselect => {
            ...
            let result = builder.ins().bitselect(a, b, c);
            state.push1(result)
        }
